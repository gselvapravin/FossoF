...
echo "Building FossoF in $DEST from $SRC"
sleep 3
#--------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------
echo "------ Compiling kernel boot loader"
...
chroot $DEST/output/sdcard /bin/bash -c "apt-get -qq -y install bash-completion lsof console-data sudo git hostapd dosfstools htop openssh-server ca-certificates module-init-tools dhcp3-client udev ifupdown iproute iputils-ping ntpdate ntp rsync usbutils uboot-envtools pciutils wireless-tools wpasupplicant procps libnl-dev parted cpufrequtils console-setup unzip bridge-utils" 
...
chroot $DEST/output/sdcard /bin/bash -c "(echo admin123;echo admin123;) | passwd root" 

# set hostname 
echo fossof > $DEST/output/sdcard/etc/hostname
